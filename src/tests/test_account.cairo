...
use circuit_breaker::account::interface::{
    IAccount, ISecureAccount, ERC1271_VALIDATED, IACCOUNT_ID,
};
...
fn test_constructor() {
    ...
    account
        .initializer(
            signer_public_key_: SIGNER_PUBLIC_KEY, guardian_public_key_: GUARDIAN_PUBLIC_KEY
        );

    assert(account.get_signer_public_key() == SIGNER_PUBLIC_KEY, 'Should return signer pubkey');
    assert(
        account.get_guardian_public_key() == GUARDIAN_PUBLIC_KEY, 'Should return guardian pubkey'
    );
}
...
fn test_validate_deploy() {
    ...
    assert(
        account
            .__validate_deploy__(
                CLASS_HASH(), SALT, SIGNER_PUBLIC_KEY, GUARDIAN_PUBLIC_KEY
            ) == starknet::VALIDATED,
...
fn test_trigger_signer_escape() {
    ...
    assert(
        account.get_signer_escape_activation_date() == BLOCK_TIMESTAMP()
            + Account::ESCAPE_SECURITY_PERIOD,
...
#[available_gas(20000000)]
#[should_panic(
    expected: ('Account: invalid implementation', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED')
)]
fn test_upgrade_invalid_implementation() {
    ...
    let call = starknet::account::Call {
        to: account.contract_address, selector: UPGRADE_SELECTOR, calldata: calldata
    };
...
fn test__is_valid_signature() {
    ...
    let is_valid = account
        ._is_valid_signature(message, invalid_length_signature.span(), data.public_key);
...
fn test_execute_with_version(version: Option<felt252>) {
    ...
    let call = starknet::account::Call {
        to: erc20.contract_address, selector: TRANSFER_SELECTOR, calldata: calldata
    };
    let mut calls = ArrayTrait::new();
    calls.append(call);
