...
mod Account {
    ...
    #[constructor]
    fn constructor(
        ref self: ContractState, signer_public_key_: felt252, guardian_public_key_: felt252
    ) {
    ...
    impl AccountImpl of account::interface::IAccount<ContractState> {
        ...
        fn is_valid_signature(
            self: @ContractState, message: felt252, signature: Span<felt252>
        ) -> u32 {
            if (self
                ._is_valid_signature(
                    :message, :signature, public_key: self._signer_public_key.read()
                )) {
        ...
        fn supports_interface(self: @ContractState, interface_id: u32) -> bool {
            if (interface_id == account::interface::IACCOUNT_ID) {
                ...
                ERC165::ERC165Impl::supports_interface(
                    self: @ERC165::unsafe_new_contract_state(), :interface_id
                )
            }
        }
        ...
        fn __execute__(
            ref self: ContractState, calls: Array<starknet::account::Call>
        ) -> Array<Span<felt252>> {
            // Modifiers
            self._non_reentrant();
        ...
        fn set_signer_public_key(ref self: ContractState, new_public_key: felt252) {
            ...
            // Events
            self.emit(Event::SignerPublicKeyChanged(SignerPublicKeyChanged { new_public_key }));
        }
    }
    ...
    impl SecureAccount of account::interface::ISecureAccount<ContractState> {
        ...
        fn set_guardian_public_key(ref self: ContractState, new_public_key: felt252) {
            ...
            // Events
            self.emit(Event::GuardianPublicKeyChanged(GuardianPublicKeyChanged { new_public_key }));
        }
        ...
        fn trigger_signer_escape(ref self: ContractState) {
            ...
            self
                .emit(
                    Event::SignerEscapeTriggered(SignerEscapeTriggered { active_at: active_date })
                );
        }
    ...
    impl HelperImpl of HelperTrait {
            ...
            self
                .emit(
                    Event::AccountInitialized(
                        AccountInitialized {
                            signer_public_key: signer_public_key_,
                            guardian_public_key: guardian_public_key_
                        }
                    )
                );
        }
        ...
        fn _validate_transaction_with_calls(
            ref self: ContractState, calls: Array<starknet::account::Call>
        ) -> felt252 {
            let tx_info = starknet::get_tx_info().unbox();
            ...
            if (calls.len() == 1) {
                if (*calls.at(0).to == account_contract_address) {
                    let guardian_condition = (*calls.at(0).selector - ESCAPE_SIGNER_SELECTOR)
                        * (*calls.at(0).selector - TRIGGER_ESCAPE_SIGNER_SELECTOR);
            ...
            // signature check
            assert(
                self._is_valid_signature(message: tx_hash, :signature, :public_key),
        ...
        fn _validate_transaction(self: @ContractState) -> felt252 {
            ...
            assert(
                self
                    ._is_valid_signature(
                        message: tx_hash, :signature, public_key: self._signer_public_key.read()
                    ),
                'Account: invalid signature'
            );
        ...
        fn _is_valid_signature(
            self: @ContractState, message: felt252, signature: Span<felt252>, public_key: felt252
        ) -> bool {
        ...
        fn _execute_calls(
            self: @ContractState, mut calls: Array<starknet::account::Call>
        ) -> Array<Span<felt252>> {
        ...
        fn _execute_single_call(
            self: @ContractState, call: starknet::account::Call
        ) -> Span<felt252> {
